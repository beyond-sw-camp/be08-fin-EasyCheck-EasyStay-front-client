...
<template>
  <router-link :to="{ path: `/accommodation/${accommodationId}` }">
    <div class="card move-on-hover" v-bind="$attrs" :data-bs-toggle="pro ? 'tooltip' : null"
      ...
      <img class="m-auto image-container" :class="pro && 'opacity-6'" :src="image" :alt="title" loading="lazy" />
    ...
    <div class="mt-2 ms-2">
      <h6 class="image-title mb-0">{{ title }}</h6>
